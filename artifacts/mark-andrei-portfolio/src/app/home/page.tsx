    ...
    <main className="site-shell min-h-screen text-white antialiased">
      ...
      <div className="site-content-layer">
        ...
        <ScrollReveal className="mb-14" delayMs={20} repeat={false}>
          <section>
            <h1 className="font-display text-3xl font-black leading-[1.05] sm:text-5xl md:text-7xl">
              {heroName},
            </h1>
              ...
              <div className="mt-4 flex flex-wrap gap-3 text-sm">
                {featured.link && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1.5 font-medium text-awsOrange hover:underline"
                  >
              ...
              <div className="space-y-3 border-l border-white/20 pl-4 md:pl-6">
                {achievements.map((item, idx) => (
                  <ScrollReveal key={item.id} delayMs={140 + idx * 35}>
                    <p className="break-words text-sm leading-relaxed text-awsOrange">{item.text}</p>
                  </ScrollReveal>
                ))}
              </div>
            </section>
        ...
        <p className="text-sm text-slate-300 md:text-base">
          {contactLabel}{" "}
          <a href={`mailto:${email}`} className="text-awsOrange hover:underline">
            {email}
          </a>
        </p>
